 ...
 *
 * CLEARS ALL EXISTING DATA before seeding fresh data.
 */
...
const ComplaintTransfer = require("../models/ComplaintTransfer.js");
const ComplaintCommunication = require("../models/ComplaintCommunication.js");
const DepartmentConnection = require("../models/DepartmentConnection.js");
...
async function clearDatabase() {
  ...
  try {
    ...
    await Promise.all([
      ...
      ComplaintTransfer.deleteMany({}),
      ComplaintCommunication.deleteMany({}),
      DepartmentConnection.deleteMany({}),
    ]);
...
const DEMO_DATA = {
  departments: [
    {
      ...
      code: "ELEC",
      description:
        "Power supply, distribution, and electrical infrastructure management",
    },
    {
    ...
    ELEC: [
      {
        ...
        code: "BILL",
        description: "Electricity billing and payment processing",
      },
      {
        name: "Metering Section",
        code: "METER",
        description: "Meter installation and maintenance",
      },
      {
        name: "Outage Management",
        code: "OUTAGE",
        description: "Power outage response and restoration",
      },
    ],
    WATER: [
      {
      ...
      },
      {
        ...
        code: "LEAK",
        description: "Pipeline maintenance and leak repairs",
      },
      {
    ...
    GAS: [
      {
        ...
        code: "PIPE",
        description: "Gas pipeline installation and maintenance",
      },
      {
        name: "Safety Inspection",
        code: "SAFETY",
        description: "Gas safety inspections and compliance",
      },
    ],
    MUN: [
      {
        ...
        code: "SANIT",
        description: "Waste collection and sanitation",
      },
      {
        name: "Waste Management",
        code: "WASTE",
        description: "Waste processing and disposal",
      },
      {
        name: "Public Works",
        code: "WORKS",
  ...
  complaintTemplates: [
    ...
    },
    {
      ...
      description:
        "Unusually high electricity bill this month, requesting review",
      category: "ELECTRICITY",
      priority: "LOW",
    },
    ...
    // Water Department
    {
      ...
      description: "No water supply for past 2 days in residential complex",
      category: "WATER_SUPPLY",
      priority: "HIGH",
    },
    {
      title: "Water Pipeline Leakage",
      description: "Major water leakage on Main Road causing waterlogging",
      category: "WATER_SUPPLY",
      priority: "HIGH",
    },
    {
      title: "Poor Water Quality",
      description: "Water supplied has unusual taste and color",
      category: "WATER_SUPPLY",
      priority: "MEDIUM",
    },
    ...
    // Gas Department
    {
      title: "Gas Leakage Suspected",
      description: "Strong gas smell detected near residential building",
      category: "PUBLIC_SAFETY",
      priority: "URGENT",
    },
    {
      title: "Gas Connection Request",
      description: "Request for new gas connection for residential use",
      category: "OTHER",
      priority: "LOW",
    },
    ...
    },
    {
      ...
      description: "Increasing stray dog population causing safety concerns",
      category: "PUBLIC_SAFETY",
      priority: "MEDIUM",
    },
  ],
...
const getSuperAdmin = async () => {
  try {
    ...
    if (!superAdmin) {
      console.log(
        "⚠️  No Super Admin found. Creating demo Super Admin for seeding...",
      );
      superAdmin = new User({
        role: "SUPER_ADMIN",
  ...
  for (const deptData of DEMO_DATA.departments) {
    try {
      ...
      if (!department) {
        ...
        console.log(
          `   ℹ️  Department already exists: ${deptData.name} (${deptData.code})`,
        );
      }
      ...
      console.error(
        `   ❌ Error creating department ${deptData.name}:`,
        error.message,
      );
    }
  }
    ...
    for (const subDeptData of subDepts) {
      try {
        ...
        if (!subDepartment) {
          ...
          console.log(
            `   ✅ Created sub-department: ${subDeptData.name} (${deptCode}-${subDeptData.code})`,
          );
        } else {
          console.log(
            `   ℹ️  Sub-department already exists: ${subDeptData.name} (${deptCode}-${subDeptData.code})`,
          );
        }
        ...
        console.error(
          `   ❌ Error creating sub-department ${subDeptData.name}:`,
          error.message,
        );
      }
    }
    ...
    for (const subDepartment of subDeptList) {
      try {
        ...
        if (!existingOfficer) {
          ...
          const { officerId, password } = generateOfficerCredentials(
            department,
            subDepartment,
            officerIndex,
          );
          ...
          console.log(
            `   ✅ Created officer: ${officerName} (${officerId}) - ${department.name}/${subDepartment.name}`,
          );
        } else {
          console.log(
            `   ℹ️  Officer already exists for: ${department.name}/${subDepartment.name}`,
          );
        }
        ...
        console.error(
          `   ❌ Error creating officer for ${department.name}/${subDepartment.name}:`,
          error.message,
        );
      }
    }
  ...
  for (const citizenData of DEMO_DATA.citizens) {
    try {
      ...
      if (!citizen) {
        ...
        console.log(
          `   ✅ Created citizen: ${citizenData.name} (${citizenData.mobileNumber})`,
        );
      } else {
        createdCitizens.push(citizen);
        console.log(
          `   ℹ️  Citizen already exists: ${citizenData.name} (${citizenData.mobileNumber})`,
        );
      }
    } catch (error) {
      console.error(
        `   ❌ Error creating citizen ${citizenData.name}:`,
        error.message,
      );
    }
  }
    ...
    for (const subDepartment of subDeptList) {
      ...
      for (let i = 0; i < complaintsCount; i++) {
        try {
          ...
          const template =
            DEMO_DATA.complaintTemplates[
              Math.floor(Math.random() * DEMO_DATA.complaintTemplates.length)
            ];
          ...
          const assignedOfficer = officers.find(
            (officer) =>
              officer.assignedSubDepartment.toString() ===
              subDepartment._id.toString(),
          );
          ...
          const complaint = new Complaint({
            ...
            assignedOfficer:
              status !== "PENDING" ? assignedOfficer?._id : undefined,
            location: {
          ...
          console.log(
            `   ✅ Created complaint: ${complaintNumber} - ${template.title} (${status})`,
          );
        } catch (error) {
...
const displayCredentials = (superAdmin, officerCredentials, citizens) => {
  ...
  officerCredentials.forEach((officer, index) => {
    ...
    console.log(
      `   Department: ${officer.department} / ${officer.subDepartment}`,
    );
  ...
  console.log(
    `Sub-Departments: ${Object.values(DEMO_DATA.subDepartments).flat().length}`,
  );
  console.log(`Officers: ${officerCredentials.length}`);
  console.log(`Citizens: ${citizens.length}`);
  console.log(
    `Complaints: ~${Object.values(DEMO_DATA.subDepartments).flat().length * 2.5} (mixed statuses)`,
  );
...
const seedDemoData = async () => {
  try {
    ...
    // Seed officers
    const { createdOfficers, officerCredentials } = await seedOfficers(
      superAdmin,
      departments,
      subDepartments,
    );
    ...
    // Seed complaints
    const complaints = await seedComplaints(
      citizens,
      departments,
      subDepartments,
      createdOfficers,
